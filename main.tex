...
%    \begin{figure}[!h]
%        \centering
%        \includegraphics[width=\columnwidth]{analysis_data/main_analysis_folder/all_zams_params}
%        \caption{The top row shows the distribution of mass, semimajor axis and eccentricity of binary pairs at ZAMS and DCO stage for the merged DCO binaires. The bottom row shows the selection of supernova kick paramters made by the COMPAS suite.}
%        \label{fig:all_zams_params}
%    \end{figure}

%    \subsection{Binary Black Holes (BHBH)}
%    \label{subsec:binary-black-holes-(bhbh)}
%    The number of BHBH pair detections per galaxy instance is shown in figure~\ref{fig:bhbhndetections}.
%    The mean value of selected parameters\footnote{$\mone{DCO}$, $\mtwo{DCO}$, $\semaxis{DCO}$, $\ecc{DCO}$, Z, $\interval{evol}$, $\interval{lookback}$, and SNR.} across the hundred instances of the generated MW galaxy are shown in figure~\ref{fig:bhbh_n_galaxy_mean_plot}.
...
%    The maximum starting size for the primary star was $\sim$\SI{149.836}{\Msun} with a $\sim$\SI{115.624}{\Msun} companion.
%    However, the resulting black holes from these two giant stars were only of $\sim$\SI{10.385}{\Msun} and $\sim$\SI{10.395}{\Msun} respectively.
%
%    Compared to the $\sim$\SI{10}{\Msun} BHBH pair generated from one of the largest ZAMS pair, the maximum size of the primary black hole in our data set is $\sim$\SI{43.408}{\Msun} with an equal mass companion.
%    This black hole pair was generated from an equal mass ZAMS pair as well, with starting masses of $\sim$\SI{57.334}{\Msun} each.

%    \subsection{Binary Neutron Stars (NSNS)}
%    \label{subsec:binary-neutron-stars-(nsns)}
%
%    The number of NSNS pair detection per galaxy instance is shown in figure~\ref{fig:nsnsndetections}.
%    Figure~\ref{fig:nsns_n_galaxy_mean_plot} shows the mean value of selected parameters across the hundred instances of generated MW galaxy.
%    
%    For the NSNS pairs, the maximum starting size of the primary star in the data set is $\sim$\SI{54.410}{\Msun} with a companion star of $\sim$\SI{13.760}{\Msun}.
%    This binary resulted in the formation of NSNS pair of masses, $\sim$\SI{1.614}{\Msun} and $\sim$\SI{1.235}{\Msun} respectively.
%
%    In comparison to this binary, the maximum size for primary NS is $\sim$\SI{1.938}{\Msun} with a companion NS of mass $\sim$\SI{1.487}{\Msun}.
%    The progenitor primary and secondary stars for this binary had masses, $\sim$\SI{14.022}{\Msun} and $\sim$\SI{13.938}{\Msun} respectively.
%
%	\subsection{Neutron Star Black Hole pair (NSBH)}
%    \label{subsec:neutron-star-black-hole-(nsbh)}
%	
%	The number of NSBH pair detection per galaxy instance is shown in figure~\ref{fig:nsbhndetections}.
%	Figure~\ref{fig:nsbh_n_galaxy_mean_plot} shows the mean value of selected parameters across the hundred instances of generated MW galaxy.
%
%	For the NSBH pairs, the ZAMS value for the maximum size primary star is $\sim$\SI{53.708}{\Msun} and that of the secondary star is $\sim$\SI{29.613}{\Msun}. The resulting NSBH pair from this system has the following masses, $\sim$\SI{1.439}{\Msun} and $\sim$\SI{15.342}{\Msun}.
%	
%	On the other hand, the maximum size NS in the NSBH pair came out to be $\sim$\SI{1.935}{\Msun} and the BH mass was $\sim$\SI{3.824}{\Msun}. The progenitors of this pair have the masses $\sim$\SI{14.090}{\Msun}, and $\sim$\SI{13.959}{\Msun}.
...
%	\subsection{Black Hole Neutron Star pairs (BHNS)}
%	\label{subsec:black-hole-neutron-star-paris-(bhns)}

%	\begin{figure}[!h]
%		\centering
%		\includegraphics[width=0.75\textwidth]{analysis_data/main_analysis_folder/BHNS_n_detections}
%		\caption{Number of BHNS pair detection per galaxy instance. On average, a total of $\sim$32 pairs per galaxy were detected in this study.}
