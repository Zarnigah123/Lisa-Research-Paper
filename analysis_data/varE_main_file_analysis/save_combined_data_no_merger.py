...
df = pd.DataFrame([dco_values['Merges_Hubble_Time'][()], sys_seeds, weights, m1_zams, m1_dco, m2_zams, m2_dco, a_zams, a_dco, e_zams, e_dco, z_zams, stype_sys1, stype1, stype_sys2, stype2, km1_zams, km2_zams, kp1_zams, kp2_zams, kt1_zams, kt2_zams, kma1_zams, kma2_zams]).T

df.columns = ['Merges', 'Seed', 'Weight', 'Mass@ZAMS(1)', 'Mass@DCO(1)', 'Mass@ZAMS(2)', 'Mass@DCO(2)', 'SemiMajorAxis@ZAMS', 'SemiMajorAxis@DCO', 'Eccentricity@ZAMS', 'Eccentricity@DCO', 'Metallicity@ZAMS', 'StellarType@ZAMS(1)', 'StellarType@DCO(1)', 'StellarType@ZAMS(2)', 'StellarType@DCO(2)', 'KickMagnitude(1)', 'KickMagnitude(2)', 'KickPhi(1)', 'KickPhi(2)', 'KickTheta(1)', 'KickTheta(2)', 'KickMeanAnomaly(1)', 'KickMeanAnomaly(2)']

a = np.linspace(1e-4, 0.03, 5)

df.loc[:, 'cond'] = np.zeros(len(df['Metallicity@ZAMS']))
...
combined_['Seed'] = combined_['Seed'].apply(int)
combined_ = combined_.sort_values(by='Seed')
combined_.reset_index(drop=True, inplace=True)

combined_.to_csv('./combined_dataset.csv', header=True, index=False)
