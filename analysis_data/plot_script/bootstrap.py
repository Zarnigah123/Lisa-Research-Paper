"""
Created on Fri Jan  7 14:17:55 2022
...
def bootstrapped_kde(variable, weights, seeds, ax, bw_adjust=None, normalisation=1,
                     lower_bound=None, upper_bound=None, second=True,
    ...
    for i in range(bootstraps):
        ...
        inds = np.array([np.random.choice(r) for r in res])

        loop_variable = variable[inds]
        loop_variable = loop_variable.reset_index(drop=True)

        loop_weights = weights[inds] * counts
        loop_weights = loop_weights.reset_index(drop=True)
