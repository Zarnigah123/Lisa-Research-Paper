...
for i in range(len(dco_types)):
    with h5.File(f'{fnc.ANALYSIS_DATA_PATH}/{dco_types[i]}0e.h5', "r") as f:
...
s_df = pd.DataFrame([all_f_dom.value, all_asd.to(u.Hz ** -0.5).value, all_f.value, all_ecc, all_Z, all_types,
                     all_snr, all_harmonics, all_seed, all_galaxy]).T
s_df.columns = ['f_dom', 'ASD', 'f_orb', 'ecc', 'Z', 'types', 'SNR', 'max_snr_harmonic', 'seeds', 'gal_number']
...
p = sns.scatterplot(data=s_df, x='f_dom', y='ASD', hue='ecc', size='Z', palette='RdYlGn_r', ax=ax, lw=0, ec="k",
                    zorder=2, style='types')
...
plt.tight_layout()
plt.savefig(f'{fnc.IMG_PATH}/all_dco_snr_plotting0e.pdf')
plt.savefig(f'{fnc.IMG_PATH}/all_dco_snr_plotting0e.png')
plt.close()
...
plt.tight_layout()
fnc.save_figure(pyplot_object=plt, fig_name='dco_typewise_snr0e')
plt.close()
