...
for i in range(len(dco_types)):
    with h5.File(f'{fnc.ANALYSIS_DATA_PATH}/{dco_types[i]}.h5', "r") as f:
...
plt.tight_layout()
plt.savefig(f'{fnc.IMG_PATH}/all_dco_snr_plotting.pdf')
plt.savefig(f'{fnc.IMG_PATH}/all_dco_snr_plotting.png')
plt.close()
...
plt.tight_layout()
fnc.save_figure(pyplot_object=plt, fig_name='dco_typewise_snr')
plt.close()
