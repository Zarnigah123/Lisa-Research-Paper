...
def average_plotting(dco_type):
    evolved_ = h5.File(f'{fnc.ANALYSIS_DATA_PATH}/{dco_type}0e.h5')['simulation'][()]

    evolved_ = pd.DataFrame(evolved_)

    evolved_ = evolved_[['m1_dco', 'm2_dco', 'm_chirp', 'a_dco', 'e_dco', 'Z', 't_evol',
                         'lookback_time', 'SNR', 'SNR_harmonics', 'galaxy_number', 'seed']]
    ...
    plt.tight_layout()
    # fig_name = f'{dco_type}0e_n_galaxy_mean_plot'
    ...
    # fig_name = f'{dco_type}0e_n_detections'
    # fnc.save_figure(pyplot_object=plt, fig_name=fig_name)
    # plt.close()


# [average_plotting(i) for i in ['BHBH', 'NSNS', 'BHNS', 'NSBH']]
average_plotting('NSNS')
