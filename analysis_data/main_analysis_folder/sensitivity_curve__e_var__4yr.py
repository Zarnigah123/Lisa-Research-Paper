...
# sns.kdeplot(x=np.log10(all_f_dom.value), hue=all_types, ax=ax[1], fill=True)
# plt.tight_layout()
...
s_df.f_dom = s_df.f_dom.apply(lambda x: x.value)
s_df_bhbh.f_dom = s_df_bhbh.f_dom.apply(lambda x: x.value)
s_df_bhns.f_dom = s_df_bhns.f_dom.apply(lambda x: x.value)
...
p = sns.scatterplot(x='f_dom', y='ASD', hue='ecc', data=s_df_nsns, ax=axes1[1], palette='viridis',
                    size='Z',
...
handles, labels = p.get_legend_handles_labels()
p.legend_.remove()
axes1[1].legend(handles[5:-1], labels[5:-1], loc='best')
...
p = sns.scatterplot(x='f_dom', y='ASD', hue='ecc', data=s_df_bhns, ax=axes2[0], palette='viridis',
                    size='Z',
                    sizes=(50, 200), style=['BHNS'] * len(fid_sources[2]['m1_dco']), markers='*')
...
handles, labels = p.get_legend_handles_labels()
axes2[0].legend(handles[5:-1], labels[5:-1], loc='best')
...
p = sns.scatterplot(x='f_dom', y='ASD', hue='ecc', data=s_df_nsbh, ax=axes2[1], palette='viridis',
                    size='Z',
...
handles, labels = p.get_legend_handles_labels()
p.legend_.remove()
axes2[1].legend(handles[5:-1], labels[5:-1], loc='best')
