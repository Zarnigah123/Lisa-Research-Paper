...
def get_COMPAS_vars(compas_file, group, variables, mask=None,metallicity=None):
...
def mask_COMPAS_data(input_file, DCO_type, flags):
    ...
    fDCO = input_file['BSE_Double_Compact_Objects.csv']

    # get the total number of binaries
    BINARIES = len(fDCO['Stellar_Type(1)'][...].squeeze())

    # store the stellar type of both stars
    type1 = fDCO['Stellar_Type(1)'][...].squeeze()
    type2 = fDCO['Stellar_Type(2)'][...].squeeze()
    ...
    if DCO_type == "ALL":
        ...
        type_mask = np.logical_and(type1 == 14, type2 == 13)
    elif DCO_type == 'NSBH':
        type_mask = np.logical_and(type1 == 13, type2 == 14)
    else:
    ...
    if hubble:
        hubble_mask = fDCO['Merges_Hubble_Time'][...].squeeze()
    else:
